...
# Now we will continue with the models as before, but using the PCA-reduced data
...
svc_y_pred_prob = svc.predict_proba(X_test_pca)[:,1]
...
random_forest.fit(X_train_pca, y_train_res)

# Make predictions with Random Forest
random_forest_y_pred_prob = random_forest.predict_proba(X_test_pca)[:,1]
random_forest_y_pred = random_forest.predict(X_test_pca)
...
adaboost_y_pred_prob = adaboost.predict_proba(X_test_pca)[:,1]
adaboost_y_pred = adaboost.predict(X_test_pca)
