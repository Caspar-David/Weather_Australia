...
# TO BE ENSURE THAT THE DIRECTORY EXISTS BEFORE SAVING DATA CHUNKS
...
# Create small data chunks for future testing
# Save a mini chunk for API prediction testing (10-20 rows)
api_test_chunk = df.sample(n=15, random_state=42)  # 15 rows for API testing
api_test_chunk.to_csv("data/processed/api_test_chunk.csv", index=False)

# Save a mini chunk for model prediction testing (2 rows)
model_test_chunk = df.sample(n=2, random_state=42)  # 2 rows for model testing
model_test_chunk.to_csv("data/processed/model_test_chunk.csv", index=False)

# Save a mini random validation chunk (100-200 rows)
validation_chunk = df.sample(n=150, random_state=42)  # 150 rows for validation
validation_chunk.to_csv("data/processed/validation_chunk.csv", index=False)
